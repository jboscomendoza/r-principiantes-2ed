...
### Un recordatorio sobre vectorización {#sec-apply-vectorizacion}
Para entender más fácilmente el uso de la familia apply, recordemos lo visto en el [Capítulo @sec-estructuras-vectores-vectorizacion].
...
Lo anterior es, generalmente, preferible a escribir una operación para cada elemento o a usar un bucle **for**, como se describió en el
...
## Las funciones de la familia apply {#sec-apply-funciones}
...
### ¿Qué es MARGIN? {#sec-apply-apply-margin}
Recuerda que las matrices y los data frames están formadas por vectores y que estas estructuras tienen dos dimensiones, ordenadas en renglones y columnas. Esto lo vimos en el [Capítulo @sec-estructuras-matrices] y el [Capítulo @sec-estructuras-dataframes].
...
### ¿Qué es FUN? {#sec-apply-apply-fun}
...
### ¿Cómo sabe FUN cuáles son sus argumentos? {#sec-apply-apply-args}
...
### ¿Qué pasa si deseamos utilizar los demás argumentos de una función con apply?{#sec-apply-apply-otros}
...
### ¿Qué tipo de resultados devuelve apply? {#sec-apply-apply-resultados}
...
### Usando lapply() {#sec-apply-lapply-uso}
...
Esto es muy conveniente, pues la recomendación para almacenar datos en un data frame es que cada columna represente una variable y cada renglón un caso, que corresponde al enfoque **tidy** [@wickham2014]. Por lo tanto, con `lapply()` podemos manipular y transformar datos por variable.
...
### Usando lapply() en lugar de un bucle for {#sec-apply-lapply-for}
...
### Usando lapply con listas {#sec-apply-lapply-listas}
