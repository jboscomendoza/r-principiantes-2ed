# Estructuras de control {#sec-control}
...
### Usando if y else {#sec-control-if-uso}
...
Por supuesto, como lo vimos en el [Capítulo @sec-sec-funciones], podemos hacer aún más compleja la función `promedio()`, pero esto es suficiente para conocer mejor las aplicaciones de `if` `else`.

### ifelse {#sec-control-if-ifelse}
...
### Usando for {#sec-control-for-uso}
...
### for y vectorización {#sec-control-for-vectorizacion}
Notarás que el resultado que obtuvimos usando **for** es el mismo que si vectorizamos la operación.
```{r}
dado ^ 2
```

Dado que en R contamos con vectorización de operaciones, que podemos usar las funciones de la familia apply (revisado en el [Capítulo @sec-apply]) en objetos diferentes a vectores y que la manera de recuperar los resultados de un `for` es un tanto laboriosa, este tipo de bucle no es muy popular en R. 
...
### Usando while {#sec-control-while-uso}
...
## break y next {#sec-control-breaknext}
...
### Usando break {#sec-control-break-uso}
...
### Usando next {#sec-control-next-uso}
