# Subconjuntos {#sec-subconjuntos}

En R, podemos extraer partes de una estructura de datos y a estas las denominamos como **subconjuntos**. De este modo podemos seleccionar datos con características específicas que forman parte de un todo, por ejemplo, todos los valores mayores a `0` en un vector numérico o todo los que son exactamente `9`.

Para obtener subconjuntos hacemos uso de índices, operadores lógicos y álgebra Booleana mediante operadores lógicos, además de que contamos con varios procedimientos para ello. Algunos de estos procedimientos pueden usarse con cualquier estructura de datos y otros sólo funcionan con algunas de ellas.
...
Usar índices para es el procedimiento más universal para obtener subconjuntos en R porque  funciona para todas las estructuras de datos.

**Un índice en R representa una posición**. Cuando usamos índices, le pedimos a R que extraiga los datos que se encuentran en una o varias posiciones específicas dentro de una estructura de datos. A diferencia de la mayoría de los lenguajes de programación, los índices en R empiezan en **1** en lugar de **0**, de este modo, el índice del primer elemento de una estructura de datos siempre es 1, el segundo 2, y así sucesivamente.
...
De este modo, si tenemos un objeto con cuatro elementos y queremos obtener el elemento en su primera posición hacemos lo siguiente.
...
Si queremos extraer su tercer elemento hacemos lo siguiente.

```{r ejemplo_indice_3}
un_vector[3]
```

También podemos extraer más de un elemento al mismo tiempo. Si queremos extraer del segundo al cuarto elemento hacemos lo siguiente, usando una secuencia numérica

::: {.callout-note}
Para crear secuencias numéricas usamos

`numero_inicio:numero_final`

Por ejemplo, una secuencia del 1 al 10

`1:10`

Y la secuencia del 3 al 25

`3:25`
:::

```{r ejemplo_indice_secuencia}
un_vector[2:4]
```

Para extraer múltiples elementos no es necesario que estos estén uno al lado del otro en la estructura de datos, podemos elegir posiciones específicas usando vectores. Si queremos extraer el primer y cuarto elemento hacemos lo siguiente.

```{r ejemplo_indice_vector}
un_vector[c(1, 4)]
```

Es muy importante tener en cuenta que al extraer un subconjunto de una estructura de datos usando corchetes, obtenemos como resultado un objeto que tiene esa misma estructura de datos. 

Por ejemplo, si extraemos un subconjunto de una lista, el resultado será también una lista. 

```{r ejemplo_lista_es_lista}
una_lista <- list("a", "b", "c", "d")
una_lista[1]
class(una_lista[1])
```

Aunque el primer elemento de la lista es un vector de tipo cadena de texto, nuestro resultado es una lista de extensión que lo contiene como único elemento.

Esto nos muestra que el uso de índices tiene consideraciones particulares para las distintas estructuras de datos, así que revisaremos este procedimiento para cada una de ellas.

### Vectores

Empecemos creando un vector de cadena de texto que contiene los nombres de distintos niveles educativos.
...
Sin embargo, intentamos extraer el segundo y quinto elemento del vector corriendo lo siguiente, obtendremos un error.
...
Recuerda que en R, un número sencillo es también un vector, por lo tanto, cuando escribimos `un_vector[3]`, en realidad estamos dando como índice un vector que contiene al número 3, es equivalente `un_vector[c(3)]`. Podemos comprobarlo con nuestro vector `nivel`.
...
Aplicando lo anterior, si escribimos dentro de los corchetes `c(2, 5)`, tendremos éxito al extraer el segundo y quinto elemento de `nivel`.

```{r nivel_subset_vector}
nivel[c(2, 5)]
```
...
```{r mi_df}
mi_df <- data.frame(
    "nombre" = c("Armando", "Elsa", "Ignacio", "Olga"),
    "edad" = c(20, 24, 22, 30),
    "sexo" = c("H", "M", "M", "H"),
    "grupo" = c(0, 1, 1, 0)
    )
...
Recuerda que **un data frame está formado por vectores**. Como vimos en el [Capítulo @sec-estructuras-dataframes], una manera de generar data frames es combinar vectores. Estos vectores, aunque estén contenidos dentro de un data frame, conservan todas las características de un vector y es posible extraerlos como tales.
...
# Subconjunto de un subconjunto: Lista.
mi_lista[["cuatro"]][2]
```
...
Para este procedimiento usamos operadores lógicos y condicionales, como revisamos en el [Capítulo @sec-operadores] y lo podemos aplicar a **data frames**.
...
Necesitamos extraer un vector, porque aplicaremos a todos los elementos de esa columna una operación relacional, usando vectorización, como lo vimos en el [Capítulo @sec-estructuras-vector].
...
```{r subset_col_invalida, error=TRUE}
